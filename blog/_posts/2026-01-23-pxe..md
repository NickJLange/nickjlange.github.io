...
It’s been a few years since I bought a non‑Apple, non‑IoT device. Enter the Z1, snagged on Black Friday and finally delivered last week. One reason I went HP (think AppleCare) is the warranty. HP also ships something called Wolf Security 🐺 (no idea what it really does), so instead of poking at that, I decided to wipe the pre‑installed Ubuntu OS. UEFI USB Boot Failed  - how about PXE? 

This should be simple, right? My AI buddy can whip up a PXE server in seconds… right? 🤖

**FAIL**

### Lessons Learned 🧠
- HP “iLO” wasn’t what I paid $50 for — so that was fun
...
- Passing broadcast packets from `src_net` to a container *while preserving the MAC address* is tricky  
  - Thought `--net=host` would solve it. It didn’t.
- Rootless containers + dnsmasq for TFTP = 🚫  
  - Port 69 is hard‑coded for TFTP  
  - **Implication:** don’t use dnsmasq for TFTP
- Don’t let the AI buddy build the PXE server for you  
  - Fancy ideas like “download ISO on demand” wasted time  
...
- More dnsmasq surprises 🙃

If I had more time, I’d split dnsmasq into two instances:
- One for DHCP, using `--net=host` to avoid IP mangling headaches
- One for TFTP, just to remap the privileged port

## Server BOM 
- Ubuntu Linux 24.04
- HP Z2 Mini G1a w/AMD Ryzen™ AI Max+ PRO 395 (3 GHz w/up to 5.1 GHz w/Boost, 16 core), Radeon 8060S Graphics + 128 GB memory
